...
class Support_Vector_Machine:
    ...
    def visualize(self):
        ...
        #hyperplave value =x.w+b
        #v = x.w+b
        #psv = 1
        #nsv = -1
        #dec = 0
        def hyperplane(x, w, b, v): #value:v
            return (-w[0]*x-b+v) / w[1]

        datarange = (self.min_features_value-0.9, self.max_feature_value*1.1)
...
data_dict = {-1:np.array([[1,7],
                        [2,8],           #max 8 so 10*8 =80 so range [-80,80]
                        [3,8],]),
            1:np.array([[5,1],
                        [6,-1],
                        [7,3],])}
