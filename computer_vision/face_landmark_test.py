...
class Network(nn.Module):
    def __init__(self, num_classes=136):
        super().__init__()
        self.model_name = 'restnet18'
        self.model =  models.resnet18(pretrained= False)
        self.model.conv1 = nn.Conv1(1, 64, kernle_size=7, stride=2, padding=3, bias= False)
...
for (x, y, w, h) in faces:
    ...
    with torch.no_grad():  #skips the gradient calculation over the weights(you are not changing any weight in the specified layers)
        landmarks = best_networks(image.unsqueeze(0))

    landmarks = (landmarks.view(68, 2).detch().numpy() +0.5) * np.array([[w, h]])+ np.array([[x, y]])
